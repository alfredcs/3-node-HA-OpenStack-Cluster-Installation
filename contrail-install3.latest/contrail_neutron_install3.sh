...
while getopts "ho:F:Lm:n:M:R:S:t:T:v:V:D" OPTION; do
case "$OPTION" in
h)
        usage
        exit 0
...
o)
	...
	[[ ! ${openstack_controller} ]] && { usage; exit 1; }
        ;;
F)
        first_contrail_controller_name=`grep "${OPTARG}\s" /etc/hosts | grep -v ^#|head -1|awk '{print $1}'`
        ;;
L)
	HTTP_CMD=https
	SSL_FLAG="-L"
...
R)
        ...
        ;;
S)
        second_contrail_controller_name=`grep "${OPTARG}\s" /etc/hosts | grep -v ^#|head -1|awk '{print $1}'`
        ;;
t)
	...
	[[ ! ${admin_ks_init_token} ]] && { usage; exit 1; }
        ;;
T)
        third_contrail_controller_name=`grep "${OPTARG}\s" /etc/hosts | grep -v ^#|head -1|awk '{print $1}'`
        ;;
n)
...
###
yum -y erase `rpm -qa | egrep -i 'nova|keystine|glance|neutron|contrail|neutron|supervisor|python-keystoneclient|haproxy|cassandra|zookeeper'`; rpm -e --nodeps supervisor redis gmp gmp-devel python-bitarray
...
yum -y install --disablerepo=* --enablerepo=havana_install_repo110 gmp gmp-devel
[[ ! -f /usr/lib64/libgmp.so.3 ]] && ln -s /usr/lib64/libgmp.so /usr/lib64/libgmp.so.3
[[ `rpm -qa | grep contrail|wc -l` -lt 1 ]] && yum -y install --disablerepo=* --enablerepo=contrail_install_repo110 contrail-openstack-analytics \
	contrail-openstack-config contrail-openstack-database contrail-openstack-control \
	contrail-openstack-webui supervisor.x86_64 python-novaclient contrail-config redis \
